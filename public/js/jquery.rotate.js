 ...
 * - conflicting with other means of modifying transform property
 *
...
(function($) {
	...
	support.transform = divStyle.MozTransform === '' ? 'MozTransform'
	...
	$.cssHooks.rotate = {
		set : function(elem, value) {
			...
			if (supportTransform) {
				...
				// From pbakaus's Transformie
				// http://github.com/pbakaus/transformie
				if (centerOrigin = $.rotate.centerOrigin) {
					elem.style[centerOrigin == 'margin' ? 'marginLeft' : 'left'] = -(elem.offsetWidth / 2)
							+ (elem.clientWidth / 2) + "px";
					elem.style[centerOrigin == 'margin' ? 'marginTop' : 'top'] = -(elem.offsetHeight / 2)
